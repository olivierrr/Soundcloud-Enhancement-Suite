...
parseUrl = function(url) {
	...
	if(url === 'https://soundcloud.com/stream') {
		whenIsLoaded(document.querySelector('.stream__list') && document.querySelector('.streamSidebar'),
			function() {
		  		inject('groups/groups', '.streamSidebar')
		        inject('stream/stream', '.stream')
			})
	}

	// should be regEx
	if(url === 'https://soundcloud.com/olivierrr01/following') template.following()
}
...
function inject(fileLocation, domLocation) {
	chrome.runtime.sendMessage({ template: fileLocation }
	,   function(res) {
